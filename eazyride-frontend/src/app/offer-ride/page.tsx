...
export default function OfferRidePage() {
  ...
  return (
    <div className={`min-h-screen font-sans pb-20 transition-all duration-1000 bg-slate-50`}>
      ...
      <main className="max-w-4xl mx-auto px-6 mt-8">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 mb-8">
           <div>
              ...
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">Share Ride</h1>
              <p className="text-slate-900/30 text-[10px] mt-1 font-black uppercase tracking-widest leading-none">Share your route with neighbors</p>
           </div>
           
           <div className="flex items-center gap-4 bg-white border border-slate-200 rounded-3xl px-6 py-4 backdrop-blur-md">
              <ShieldCheck className="w-5 h-5 text-green-600" />
              <div>
                 <p className="text-[10px] font-black text-slate-900/30 uppercase tracking-widest">ECO IMPACT</p>
                 <p className="font-black text-slate-900 leading-none">{carbonSaved}kg CO₂ saved</p>
              </div>
           </div>
        </div>
        ...
        <div className="space-y-4">
           {/* DIRECTION TOGGLE */}
           <div className="bg-white border border-slate-200 p-2 rounded-[2rem] flex gap-2">
              <button 
                onClick={() => handleDirectionToggle('to_office')}
                className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-[1.5rem] font-black transition-all ${direction === 'to_office' ? 'bg-blue-600 text-slate-900 shadow-xl' : 'text-slate-900/20 hover:text-slate-900/40'}`}
              >
                 <Sun className="w-5 h-5" /> TO OFFICE
              </button>
              <button 
                onClick={() => handleDirectionToggle('to_home')}
                className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-[1.5rem] font-black transition-all ${direction === 'to_home' ? 'bg-orange-600 text-slate-900 shadow-xl' : 'text-slate-900/20 hover:text-slate-900/40'}`}
              >
           ...
           <div className="bg-white border border-slate-200 rounded-[2rem] p-6">
              <h3 className="text-[9px] font-black text-slate-900/30 uppercase tracking-[0.3em] mb-4">Select Your Route</h3>
              <div className="grid grid-cols-4 gap-2">
                 {corridors.map(c => (
                   <button 
                    key={c.id} 
                    onClick={() => handleCorridorSelect(c)}
                    className={`py-3 rounded-2xl border transition-all flex flex-col items-center gap-1.5 ${form.corridor_id === String(c.id) ? 'bg-blue-100 border-blue-600' : 'bg-white border-slate-200 hover:bg-slate-100'}`}
                   >
                     <MapPin className={`w-4 h-4 ${form.corridor_id === String(c.id) ? 'text-blue-600' : 'text-slate-900/20'}`} />
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
               ...
               <div className="bg-white border border-slate-200 rounded-[2rem] p-6 space-y-4">
                  <div>
                     <label className="text-[9px] font-black text-slate-900/30 uppercase tracking-widest pl-1 mb-2 block">Origin Point</label>
                     <input value={form.pickup_point} onChange={e => setForm(p => ({ ...p, pickup_point: e.target.value }))}
                       className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold focus:outline-none focus:border-blue-600 transition-all uppercase" />
                  </div>
                  <div>
                     <label className="text-[9px] font-black text-slate-900/30 uppercase tracking-widest pl-1 mb-2 block">Destination Point</label>
               ...
               <div className="bg-white border border-slate-200 rounded-[2rem] p-6 space-y-4">
                  <div>
                     <label className="text-[9px] font-black text-slate-900/30 uppercase tracking-widest pl-1 mb-2 block">Ride Date</label>
                     <input type="date" value={form.ride_date} onChange={e => setForm(p => ({ ...p, ride_date: e.target.value }))}
                       className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold focus:outline-none focus:border-blue-600 transition-all text-slate-900 font-black" />
                  </div>
                  <div>
                     <label className="text-[9px] font-black text-slate-900/30 uppercase tracking-widest pl-1 mb-2 block">Departure Time</label>
                     <input type="time" value={form.ride_time} onChange={e => setForm(p => ({ ...p, ride_time: e.target.value }))}
                       className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xl font-black focus:outline-none focus:border-blue-600 transition-all" />
                     {form.ride_time && (() => {
                        const h = parseInt(form.ride_time.split(':')[0])
                        let label = 'Custom Slot'
                        let color = 'text-slate-900/20'
                        if (direction === 'to_office') {
                          if (h >= 6 && h < 7) { label = 'Early Morning'; color = 'text-amber-600' }
                          else if (h >= 7 && h < 8) { label = 'Morning Commute'; color = 'text-amber-600' }
                          else if (h >= 8 && h < 9) { label = 'Peak Office Hour'; color = 'text-amber-600' }
                          else if (h >= 9 && h < 10) { label = 'Mid-Morning'; color = 'text-amber-600' }
                          else if (h >= 10 && h < 11) { label = 'Late Morning'; color = 'text-amber-600' }
                          else if (h >= 11 || h < 2) { label = 'Mid-Day'; color = 'text-amber-600' }
                        } else {
                          if (h >= 16 && h < 18) { label = 'Early Evening'; color = 'text-blue-600' }
                          else if (h >= 18 && h < 20) { label = 'Evening Return'; color = 'text-blue-600' }
                          else if (h >= 20 && h < 22) { label = 'Late Evening'; color = 'text-blue-600' }
                          else if (h >= 22 || h < 4) { label = 'Night Shift'; color = 'text-blue-600' }
                        }
               ...
               {/* SLOTS */}
               <div className="bg-white border border-slate-200 rounded-[2rem] p-6">
                  <label className="text-[9px] font-black text-slate-900/30 uppercase tracking-widest pl-1 mb-4 block">Available Slots</label>
                  <div className="grid grid-cols-2 gap-2">
                     {['1','2','3','4'].map(n => (
                       <button key={n} onClick={() => setForm(p => ({ ...p, available_seats: n }))}
                         className={`py-3 rounded-xl border-2 font-black text-sm transition-all ${form.available_seats === n ? 'bg-indigo-600 border-indigo-400 text-slate-900 shadow-lg' : 'bg-white border-slate-200 text-slate-900/20'}`}>
                          {n}
                       </button>
                     ))}
                  </div>
               ...
               {/* PER SEAT COST */}
               <div className="bg-white border border-slate-200 rounded-[2rem] p-6">
                  <label className="text-[9px] font-black text-slate-900/30 uppercase tracking-widest pl-1 mb-4 block">Per Seat (₹)</label>
                  <div className="relative mt-2">
                     <IndianRupee className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-900/20" />
                     <input type="number" value={form.price_per_seat} onChange={e => setForm(p => ({ ...p, price_per_seat: e.target.value }))}
                       className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-4 text-2xl font-black focus:outline-none focus:border-green-600 transition-all" />
                  </div>
               </div>
           </div>

           {/* SUBMIT SECTION */}
           <div className="bg-white border border-slate-200 rounded-[2rem] p-4 flex flex-col md:flex-row items-center gap-4">
              <button 
                onClick={() => setPostRoundTrip(!postRoundTrip)}
                className={`w-full md:w-auto px-8 py-5 rounded-[1.5rem] font-black text-[10px] uppercase transition-all flex items-center gap-3 ${postRoundTrip ? 'bg-blue-100 text-blue-600 border border-blue-500/30' : 'bg-blue-600 text-white/20 border border-slate-200'}`}
              >
              ...
              <button 
                ...
                disabled={loading}
                className="flex-1 w-full bg-blue-600 text-white hover:bg-blue-600 hover:text-slate-900 py-5 rounded-[1.5rem] font-black text-lg transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3"
              >
