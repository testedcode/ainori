...
function GlassPanel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-white border border-slate-200 backdrop-blur-xl rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden ${className}`}>
      {children}
    </div>
  )
}
...
export default function RideDetailPage() {
        ...
        >
          ...
          {!isAvailable && (
            <div className={`relative w-16 h-16 rounded-full p-1 transition-all duration-500 group-hover:scale-110 ${type === 'host' ? 'bg-gradient-to-tr from-amber-600 via-amber-400 to-yellow-200 shadow-[0_0_30px_rgba(251,191,36,0.5)]' : 'bg-gradient-to-tr from-blue-600 via-cyan-400 to-indigo-200 shadow-[0_0_30px_rgba(59,130,246,0.5)]'}`}>
              <div className="w-full h-full rounded-full bg-slate-900 flex items-center justify-center overflow-hidden border-2 border-slate-900 relative">
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-white pointer-events-none z-10" />
                {user?.avatar_url ? (
                  <img src={user.avatar_url} alt={name} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-lg font-black text-slate-900 relative z-20">{name[0]?.toUpperCase()}</span>
                )}
              </div>
          ...
          {isAvailable && !isSelected && (
            <div className="flex flex-col items-center animate-pulse">
              <span className="text-[8px] font-black text-blue-600 uppercase tracking-[0.2em] mb-1">VACANT</span>
              <Check className="w-5 h-5 text-green-600 drop-shadow-[0_0_8px_rgba(74,222,128,1)] animate-bounce" />
            </div>
          )}
        </button>
        <span className={`text-[6px] font-black uppercase tracking-widest ${isSelected ? 'text-green-600' : isAvailable ? 'text-slate-900/10' : 'text-slate-900/40'}`}>{name.split(' ')[0]}</span>
      </div>
    )
  }
  ...
  const StarRating = ({ rideId, rateeId, current, label, onRate }: { rideId: number, rateeId: number, current: number | null, label: string, onRate: (rating: number) => void }) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        <span className="text-[8px] font-black text-slate-900/20 uppercase tracking-[0.2em]">{label}</span>
        <div className="flex items-center gap-1.5">
          {[1, 2, 3, 4, 5].map(star => (
            ...
            >
              <Star className={`w-4 h-4 ${star <= (hover || (current || 0)) ? 'text-amber-600 fill-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]' : 'text-slate-900/10'}`} />
            </button>
          ))}
          {current && <span className="text-[10px] font-black text-amber-600 ml-1">{current}.0</span>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`min-h-screen font-sans pb-32 transition-all duration-1000 bg-slate-50`}>
      <PulseNav />
      ...
      {loading ? (
        ...
        <div className="max-w-4xl mx-auto px-6 mt-32 text-center py-20 bg-white rounded-[3rem] border border-slate-200">
          <h2 className="text-2xl font-black text-slate-900 italic uppercase tracking-tighter">Ride Not Found</h2>
          <Link href="/rides" className="text-blue-600 font-bold mt-4 block text-xs uppercase tracking-widest">Return to All Rides</Link>
        </div>
      ) : (
        <main className="max-w-4xl mx-auto px-6 mt-12 space-y-6 animate-in fade-in duration-700">
          ...
          <div className="flex items-start justify-between gap-4 mb-8">
            <div className="flex items-center gap-4">
              <Link href="/rides" className="p-3 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 transition-all active:scale-95">
                <ArrowLeft className="w-5 h-5 text-slate-900/40" />
              </Link>
              <div>
                <h1 className="text-4xl font-black tracking-tighter text-slate-900 leading-none uppercase italic">{ride.corridor_name}</h1>
                <div className="flex items-center gap-2 mt-2">
                  <button
                    onClick={copyId}
                    className="flex items-center gap-1.5 px-3 py-1 bg-white border border-slate-200 rounded-full hover:bg-slate-100 transition-all group"
                  >
                    <span className="text-[10px] font-black text-slate-900/40 uppercase tracking-widest">Ride ID</span>
                    <span className="text-[11px] font-mono font-black text-blue-600">#{rideId}</span>
                    <Copy className="w-3 h-3 text-slate-900/20 group-hover:text-blue-600 transition-colors" />
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-amber-400/10 rounded-2xl flex items-center justify-center">
                    <Star className="w-6 h-6 text-amber-600 animate-pulse" />
                  </div>
                  <div>
                    <h3 className="text-lg font-black text-slate-900">Rate your trip</h3>
                    <p className="text-xs text-slate-900/40 font-bold uppercase tracking-widest">Help others by sharing your experience</p>
                  </div>
                </div>

                <div className="flex flex-wrap items-center gap-8 bg-white p-4 rounded-3xl border border-slate-200">
                  {/* Rider rates Host */}
                  {!isOwner && isAccepted && (
                    <StarRating
                      rideId={ride.id}
                  ...
                  {isOwner && (!ride.confirmed_riders || ride.confirmed_riders.length === 0) && (
                    <p className="text-[10px] font-black text-slate-900/20 uppercase tracking-widest italic">No confirmed riders to rate</p>
                  )}
                </div>
              </div>
            </GlassPanel>
          ...
          <GlassPanel className="border-blue-500/20 !p-0 overflow-hidden">
            ...
            {ride.vehicle_info?.image_url && (
              <div className="absolute inset-0 opacity-10">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-black via-black/80 to-white" />
              </div>
            )}
            <div className="relative p-8 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-5">
                <div className="relative flex-shrink-0">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-2xl font-black overflow-hidden border-2 border-slate-200">
                ...
                <div>
                  <p className="text-[48px] font-black text-slate-900 leading-none tracking-tighter">{ride.ride_time.slice(0, 5)}</p>
                  <p className="text-[10px] font-black uppercase tracking-widest mt-1">
                    <span className={isOwner ? 'text-amber-600' : 'text-blue-600'}>
                      {isOwner ? '⭐ You are hosting' : `Host: ${ride.user_name}`}
                    </span>
                    <span className="text-slate-900/20 mx-2">·</span>
                    <span className={ride.direction === 'to_office' ? 'text-cyan-600' : 'text-green-600'}>
                      {ride.direction === 'to_office' ? '🏢 To Office' : '🏠 To Home'}
                    </span>
                  </p>
                  {/* Vehicle Badge */}
                  {ride.vehicle_info && (
                    <div className="mt-3 flex items-center gap-2 p-2 bg-white border border-slate-200 rounded-xl w-fit">
                      <span className="text-sm">
                        {ride.vehicle_info.vehicle_type === 'bike' ? '🏍️' : ride.vehicle_info.vehicle_type === 'suv' ? '🚙' : '🚗'}
                      </span>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-900/60 leading-none uppercase tracking-tighter">
                          {ride.vehicle_info.make} {ride.vehicle_info.model}
                        </span>
                        <span className="text-[8px] font-black text-slate-900/30 leading-none uppercase tracking-widest mt-0.5">
              ...
              <div className="text-center md:text-right space-y-3">
                <p className="text-sm font-black text-slate-900/60 uppercase tracking-widest">
                ...
                <div className="flex items-center gap-2 justify-end">
                  ...
                  <span className="text-xl font-black text-slate-900">{ride.price_per_seat}</span>
                  <span className="text-[10px] text-slate-900/30 font-black uppercase">/seat</span>
                </div>
                {/* Seat progress bar */}
                <div className="w-full md:w-40">
                  <div className="flex justify-between mb-1">
                    <span className="text-[9px] text-slate-900/30 uppercase font-black">Rider Occupancy</span>
                    <span className="text-[9px] font-black text-slate-900">{(ride.total_seats - 1) - ride.available_seats} / {ride.total_seats - 1} filled</span>
                  </div>
          ...
          <GlassPanel className="border-blue-500/20 !p-8">
            <div className="relative">
              <div className="flex items-center justify-between relative z-10">
                {[
                  { id: 'starting', label: 'Starting', icon: PlayCircle, color: 'text-blue-600', bg: 'bg-blue-400/20' },
                  { id: 'at_pickup', label: 'At Pickup', icon: Navigation2, color: 'text-cyan-600', bg: 'bg-cyan-400/20' },
                  { id: 'completed', label: 'Finished', icon: Flag, color: 'text-green-600', bg: 'bg-green-400/20' }
                ].map((step, idx, arr) => {
                  ...
                  return (
                    <div key={step.id} className="flex flex-col items-center gap-3 relative z-10">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                        ...
                        'bg-blue-600 text-white/20 border border-slate-200'
                      }`}>
                        <step.icon className={`w-6 h-6 ${isActive ? 'animate-pulse' : ''}`} />
                        {isPast && <Check className="w-3 h-3 absolute top-1 right-1" />}
                      </div>
                      <span className={`text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-slate-900' : 'text-slate-900/20'}`}>
                        {step.label}
                      </span>
                    </div>
                  );
              ...
              {/* Progress Line */}
              <div className="absolute top-7 left-7 right-7 h-[2px] bg-white -z-0">
                <div 
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {ride.status !== 'starting' && ride.status !== 'at_pickup' && (
                  <button
                    ...
                    className="flex-1 min-w-[140px] py-4 bg-blue-600 hover:bg-blue-500 text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                ...
                {ride.status === 'starting' && (
                  <button
                    ...
                    disabled={updatingStage}
                    className="flex-1 min-w-[140px] py-4 bg-cyan-600 hover:bg-cyan-500 text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(8,145,178,0.2)]"
                  >
                ...
                {(ride.status === 'at_pickup' || ride.status === 'starting' || (() => {
                  ...
                  <button
                    ...
                    disabled={updatingStage}
                    className="flex-1 min-w-[140px] py-4 bg-green-600/20 border border-green-500/30 text-green-600 hover:bg-green-600 hover:text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                ...
                <button
                  ...
                  className="flex-1 min-w-[140px] py-4 bg-red-600/10 border border-red-500/20 text-red-400 hover:bg-red-600 hover:text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2"
                >
              ...
              <div className="mt-8 space-y-3">
                {!riderAtSpot ? (
                  <button
                    ...
                    className="w-full py-4 bg-blue-100 border border-blue-500/30 text-blue-600 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-slate-900 transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <div className="flex items-center gap-2 px-4 py-3 bg-blue-500/10 border border-blue-500/20 rounded-2xl">
                    ...
                    <span className="text-[11px] font-black text-blue-600 uppercase tracking-widest">At pickup — host notified</span>
                  </div>
                )}
                ...
                {!riderMarkedComplete && (
                  <button
                    ...
                    className="w-full py-4 bg-green-600/20 border border-green-500/30 text-green-600 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-green-600 hover:text-slate-900 transition-all flex items-center justify-center gap-2"
                  >
                ...
                {riderMarkedComplete && (
                  <div className="flex items-center gap-2 px-4 py-3 bg-green-500/10 border border-green-500/20 rounded-2xl">
                    ...
                    <span className="text-[11px] font-black text-green-600 uppercase tracking-widest">You've marked this ride complete ✓</span>
                  </div>
                )}
              </div>
            )}
          ...
          {isOwner && (
            <GlassPanel className="border-amber-300">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900">Seat Requests</h3>
                    <p className="text-[10px] font-black text-slate-900/30 uppercase tracking-widest">
                ...
                <div className="flex items-center gap-2 bg-white border border-amber-300 rounded-2xl px-3 py-2">
                  <span className="text-[9px] font-black text-amber-600 uppercase tracking-widest mr-1">Open Seats</span>
                  <button
                    onClick={() => handleUpdateAvailableSeats(ride.available_seats - 1)}
                    disabled={updatingSeats || ride.available_seats <= 0}
                    className="w-7 h-7 rounded-xl bg-white hover:bg-red-500/20 border border-slate-200 flex items-center justify-center text-slate-900 font-black transition-all disabled:opacity-30"
                  >−</button>
                  <span className="text-lg font-black text-slate-900 w-6 text-center">{ride.available_seats}</span>
                  <button
                    onClick={() => handleUpdateAvailableSeats(ride.available_seats + 1)}
                    disabled={updatingSeats || ride.available_seats >= ride.total_seats}
                    className="w-7 h-7 rounded-xl bg-white hover:bg-green-500/20 border border-slate-200 flex items-center justify-center text-slate-900 font-black transition-all disabled:opacity-30"
                  >+</button>
                  {updatingSeats && <Loader2 className="w-3 h-3 text-amber-600 animate-spin ml-1" />}
                </div>
              </div>
                  ...
                  {requests.map(req => (
                    <div
                      ...
                      className={`flex items-center justify-between gap-4 p-4 rounded-2xl border transition-all ${req.status === 'accepted' ? 'bg-green-500/10 border-green-500/20'
                          : req.status === 'rejected' ? 'bg-white border-slate-200 opacity-40'
                            : 'bg-white border-slate-200'
                        }`}
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-2xl flex items-center justify-center text-slate-900 font-black text-sm flex-shrink-0 border-2 border-slate-200 overflow-hidden">
                          {req.avatar_url ? (
                        ...
                        <div className="min-w-0">
                          <p className="font-black text-slate-900 text-sm truncate">{req.user_name || req.rider_name}</p>
                          <p className="text-[10px] text-slate-900/30 font-bold uppercase tracking-widest mt-0.5">
                        ...
                        {req.status === 'pending' ? (
                          <>
                            <button
                              ...
                              className="px-4 py-2 bg-green-600 hover:bg-green-500 text-slate-900 rounded-xl text-xs font-black transition-all disabled:opacity-40 flex items-center gap-1.5"
                            >
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-white border border-slate-200 text-slate-900/40 hover:bg-red-600/20 hover:text-red-400 hover:border-red-500/30 rounded-xl text-xs font-black transition-all disabled:opacity-40 flex items-center gap-1.5"
                            >
                          ...
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border ${req.status === 'accepted'
                              ...
                              : 'bg-white border-slate-200 text-slate-900/20'
                            }`}>
                            {req.status}
                          </span>
                        )}
          ...
          <GlassPanel className="border-slate-200 !p-6">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 bg-blue-500/10 rounded-lg flex items-center justify-center">
                <Car className="w-4 h-4 text-blue-600" />
              </div>
              <h3 className="text-xs font-black text-slate-900/60 uppercase tracking-[0.3em]">Social Seating</h3>
            </div>

            <div className="flex flex-col items-center gap-6">
              {/* SEATING ARRANGEMENT */}
              <div className="w-full flex items-center justify-between gap-4 px-6 py-4 bg-slate-50 border border-slate-200 rounded-[2.5rem] relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-white pointer-events-none" />
                ...
                <div className="text-right min-w-[80px]">
                  <p className="text-2xl font-black text-slate-900 leading-none italic tracking-tighter">₹{ride.price_per_seat}</p>
                  <p className="text-[8px] font-black text-slate-900/20 uppercase tracking-widest mt-1">per seat</p>
                </div>
              </div>
                  ...
                  {!myRequest ? (
                    <button
                      ...
                      className={`w-full py-5 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 relative overflow-hidden group ${selectedSeats > 0
                          ? 'bg-blue-600 text-white shadow-[0_20px_40px_rgba(255,255,255,0.2)] scale-102 active:scale-95'
                          : 'bg-blue-600 text-white/20 border border-slate-200'
                        }`}
                    >
                      {selectedSeats > 0 && <div className="absolute inset-0 bg-gradient-to-r from-green-400/20 via-white to-green-400/20 animate-pulse" />}
                    ...
                    <div className="w-full h-16 rounded-[2rem] border-2 border-dashed border-slate-200 flex items-center justify-center gap-4 group">
                      {isAccepted ? (
                        <div className="flex items-center gap-2 text-green-600 font-extrabold text-xs uppercase tracking-widest">
                          <CheckCircle2 className="w-5 h-5" /> Seat Confirmed
                        </div>
                      ) : (
                        <button
                          onClick={handleCancelRequest}
                          className="flex items-center gap-2 text-blue-600 font-extrabold text-xs uppercase tracking-widest hover:text-red-400 transition-colors"
                        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <GlassPanel className="flex flex-col justify-center">
              <p className="text-[10px] font-black text-slate-900/20 uppercase tracking-widest mb-6">Route</p>
              <div className="space-y-6 relative before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-px before:bg-slate-100">
                <div className="flex items-center gap-4 relative z-10">
                  ...
                  <p className="font-black text-slate-900 uppercase text-xs">{ride.pickup_point}</p>
                </div>
                <div className="flex items-center gap-4 relative z-10">
                  <div className="w-3.5 h-3.5 rounded-full bg-slate-100 border-2 border-black" />
                  <p className="font-black text-slate-900/30 uppercase text-xs">{ride.drop_point}</p>
                </div>
              </div>
            </GlassPanel>

            <div className="h-64 rounded-[2.5rem] overflow-hidden border border-slate-200 relative group">
              <iframe
                ...
                loading="lazy"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-white pointer-events-none" />
              <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Navigation className="w-4 h-4 text-blue-600" />
                  <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Route Preview</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {ride.vehicle_info && (
            <GlassPanel className="!p-0 overflow-hidden border-slate-200 group/car">
                ...
                <div className="p-8 relative z-10 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-[10px] font-black text-slate-900/20 uppercase tracking-[0.3em]">Vehicle Details</span>
                    </div>
                    <h2 className="text-3xl font-black text-slate-900 leading-tight mb-2">
                      {ride.vehicle_info.make} <span className="text-blue-600 italic">{ride.vehicle_info.model}</span>
                    </h2>
                    <div className="flex flex-wrap gap-2 mb-6">
                      <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-black text-slate-900/40 uppercase tracking-widest">{ride.vehicle_info.color}</span>
                      <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-black text-slate-900/40 uppercase tracking-widest">{ride.vehicle_info.vehicle_type}</span>
                      <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-black text-slate-900/40 uppercase tracking-widest">{ride.vehicle_info.total_seats} Seats</span>
                    </div>
                  </div>

                  <div className="mt-4">
                    <p className="text-[10px] font-black text-slate-900/20 uppercase tracking-widest mb-2">Plate Number</p>
                    <div className="inline-flex flex-col">
                      <div className="px-6 py-3 bg-blue-600 text-white rounded-xl font-mono font-black text-xl tracking-[0.2em] shadow-[0_10px_30px_rgba(255,255,255,0.1)] border-4 border-slate-200">
            ...
            <GlassPanel className="border-green-500/20">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-900/30 font-black uppercase tracking-widest">
              ...
              <div className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-2xl mb-4">
                <span className="font-mono text-slate-900 font-bold text-sm flex-1">{ride.upi_id}</span>
                <button
                  onClick={() => { navigator.clipboard.writeText(ride.upi_id || ''); toast.success('UPI ID copied!') }}
                  className="px-3 py-1.5 bg-green-500/20 border border-green-500/30 text-green-600 rounded-xl text-[10px] font-black hover:bg-green-500/30 transition-all flex items-center gap-1"
                >
              ...
              {(ride as any).host_qr_code_url && (
                <div className="mb-4">
                  <button
                    ...
                    className="w-full py-3 bg-white border border-slate-200 rounded-2xl text-[10px] font-black text-slate-900/60 uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center gap-2"
                  >
              ...
              {isAccepted && (() => {
                ...
                return (
                  <div className="space-y-3">
                    <p className="text-[9px] font-black text-slate-900/20 uppercase tracking-widest">Pay via app</p>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <button
                          ...
                          className="flex flex-col items-center gap-1.5 p-3 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 transition-all active:scale-95"
                        >
                          <span className="text-xl">{app.emoji}</span>
                          <span className="text-[9px] font-black text-slate-900/60 uppercase tracking-wide">{app.label}</span>
                        </button>
                      ))}
                    </div>
                    {/* Mark payment done */}
                    <button
                      ...
                      className={`w-full py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                        alreadyPaid
                          ...
                          : 'bg-green-600/20 border border-green-500/30 text-green-600 hover:bg-green-600/30'
                      }`}
                    >
              ...
              {isOwner && acceptedRequests.length > 0 && (
                <div className="space-y-4 mt-6">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[10px] font-black text-slate-900/20 uppercase tracking-widest">Payment Status</p>
                    {ridePayments.filter((p: any) => p.giver_status !== 'received').length > 1 && (
                      <button 
                        onClick={handleMarkAllReceived}
                        disabled={markingAllPayment}
                        className="text-[9px] font-black text-blue-600 uppercase tracking-tighter bg-blue-400/10 px-3 py-1.5 rounded-lg border border-blue-400/20 hover:bg-blue-400/20 transition-all"
                      >
                  ...
                  {acceptedRequests.map(req => {
                    ...
                    return (
                      <div key={req.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                        isReceived ? 'bg-green-500/5 border-green-500/10 opacity-60' : 'bg-white border-slate-200'
                      }`}>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-xs font-black overflow-hidden border border-slate-200">
                            {req.avatar_url ? <img src={req.avatar_url} className="w-full h-full object-cover" alt="" /> : (req.user_name || req.rider_name || "?")[0]}
                          </div>
                          <div>
                            <p className="text-[12px] font-black text-slate-900 leading-none">{req.user_name || req.rider_name}</p>
                            <p className="text-[10px] font-black text-green-600 mt-1 uppercase tracking-widest">
                              ₹{ride.price_per_seat * req.seats_requested} <span className="text-slate-900/20">/ {req.seats_requested} seat{req.seats_requested > 1 ? 's' : ''}</span>
                            </p>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                            isReceived 
                              ...
                              : 'bg-blue-600 hover:bg-blue-500 text-slate-900 shadow-lg'
                          }`}
                        >
                ...
                messages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                      {!isMine && <span className="text-[9px] font-black text-slate-900/20 uppercase mb-1 ml-1">{msg.user_name}</span>}
                      <div className={`px-5 py-3 rounded-2xl text-sm font-medium ${isMine ? 'bg-blue-600 text-slate-900 rounded-br-none' : 'bg-blue-600 text-slate-700 border border-slate-200 rounded-bl-none'}`}>
                        {msg.message}
                      </div>
                      <span className="text-[9px] text-slate-900/10 mt-1 uppercase font-black">{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  )
                })
              )}
            ...
            <div className="p-6 bg-white border-t border-slate-200">
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 bg-white border border-slate-200 rounded-2xl px-6 py-4 text-sm font-medium focus:outline-none focus:border-blue-600 transition-all"
                />
                <button
                  onClick={sendMessage}
                  disabled={!newMessage.trim() || sendingMsg}
                  className="bg-blue-600 text-white hover:bg-blue-600 hover:text-slate-900 px-6 py-4 rounded-2xl transition-all"
                >
