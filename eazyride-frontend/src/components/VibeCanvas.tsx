...
export default function VibeCanvas({ vibe }: { vibe: VibeState }) {
  ...
  return (
    <div className={`fixed inset-0 pointer-events-none -z-20 overflow-hidden bg-slate-50`}>
      ...
      {(elements.includes('sun') || elements.includes('moon')) && (
        <div className={`absolute transition-all duration-[2000ms] ease-in-out ${
          ...
          {elements.includes('sun') && (
            <svg className="w-full h-full text-amber-600/10 animate-pulse" viewBox="0 0 100 100">
      ...
      {elements.includes('clouds') && (
        <div className="absolute inset-0">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="absolute bg-white blur-3xl rounded-full" style={{
              width: `${200 + i * 100}px`,
              height: `${100 + i * 50}px`,
              top: `${20 + i * 15}%`,
              left: `${-10 + i * 30}%`,
      ...
      {elements.includes('birds') && (
        <div className="absolute inset-0">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="absolute text-slate-900/10" style={{
              top: `${30 + i * 5}%`,
              left: `-50px`,
      ...
      <div className="absolute bottom-0 left-0 w-full h-48 opacity-[0.03] pointer-events-none z-0">
        <svg className="w-full h-full text-slate-900" preserveAspectRatio="none" viewBox="0 0 1000 100">
