...
function AccordionItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div
      className={`border rounded-2xl transition-all duration-300 cursor-pointer overflow-hidden ${open ? 'border-blue-500/30 bg-blue-600/5' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}
      onClick={() => setOpen(!open)}
    >
      <div className="flex items-center justify-between p-5 gap-4">
        <div className="flex items-start gap-3">
          <HelpCircle className={`w-4 h-4 mt-0.5 flex-shrink-0 ${open ? 'text-blue-400' : 'text-white/20'}`} />
          <p className="font-bold text-sm text-white leading-snug">{q}</p>
        </div>
        {open
      ...
      {open && (
        <div className="px-5 pb-5 pt-0">
          <div className="ml-7 text-sm text-slate-400 leading-relaxed border-l-2 border-blue-500/30 pl-4">
            {a}
          </div>
        </div>
      )}
    ...
    <div className="min-h-screen bg-[#0f172a] text-white font-sans pb-32">
          ...
          <div className="bg-gradient-to-br from-green-600/20 to-emerald-900/20 border border-green-500/20 rounded-[2rem] p-6 md:p-8 min-w-[260px] relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-3">
                ...
                <div>
                  <p className="text-[9px] font-black text-green-400 uppercase tracking-widest">Carbon Impact</p>
                  <p className="text-[9px] font-black text-white/30 uppercase tracking-wider">Every Ride Counts</p>
                </div>
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {['Fewer Solo Cars', 'Verified Routes', 'Tracked Impact'].map(tag => (
                  <span key={tag} className="px-2 py-0.5 bg-green-500/10 border border-green-500/20 text-green-400 text-[9px] font-black uppercase tracking-wider rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12">
                ...
                <div key={flow.role} className={`bg-gradient-to-br ${flow.gradient} border ${flow.border} rounded-[2.5rem] p-8 md:p-12`}>
                  <div className="flex items-center gap-4 mb-10">
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-white">{flow.role}</h3>
                    </div>
                  </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  ...
                  <div key={i} className={`bg-white/[0.03] border ${f.border} rounded-[2rem] p-6 hover:bg-white/[0.06] transition-colors group`}>
                    ...
                    <h4 className="font-black text-white mb-2">{f.title}</h4>
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
              {[
                ...
                { step: '02', title: 'Commute Together', desc: 'Complete the ride. Drop-off is marked on the ride page.', icon: Car, color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
                { step: '03', title: 'Pay via UPI', desc: "Transfer the agreed amount to the host's UPI ID shown on the ride detail page.", icon: IndianRupee, color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20' },
              ].map((s, i) => (
                <div key={i} className={`bg-white/[0.03] border ${s.border} rounded-[2rem] p-7 text-center`}>
                  ...
                  <h4 className="font-black text-white text-lg mb-2">{s.title}</h4>
            ...
            <div className="bg-white/[0.03] border border-white/10 rounded-[2rem] overflow-hidden">
              <div className="p-6 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-white">Corridor Pricing Guide</h3>
                    <p className="text-slate-500 text-xs">Minimum and suggested per-seat contributions</p>
                  </div>
                </div>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="text-left px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Corridor</th>
                      <th className="text-center px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Distance</th>
                      <th className="text-center px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Min / Seat</th>
                      <th className="text-center px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Suggested</th>
                      <th className="text-center px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Seats</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              <div className="p-5 border-t border-white/5 bg-white/[0.02]">
                <p className="text-[11px] text-slate-600 leading-relaxed">
                  <AlertCircle className="w-3 h-3 inline mr-1 text-slate-600" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-amber-500/5 border border-amber-500/20 rounded-[2rem] p-6">
                ...
                <button
                  onClick={() => setActiveTab('ticket')}
                  className="w-full py-2.5 bg-amber-400/10 border border-amber-500/30 text-amber-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-400/20 transition-colors"
                >
            ...
            <form onSubmit={handleFeedbackSubmit} className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] p-8 space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${feedbackForm.type === t.id ? 'bg-blue-600 text-white shadow-lg' : 'bg-white/5 border border-white/10 text-white/40 hover:text-white'}`}
                    >
                      {t.label}
                    </button>
                  ))}
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map(n => (
                    ...
                    >
                      <Star className={`w-7 h-7 ${n <= feedbackForm.rating ? 'fill-amber-400 text-amber-400' : 'text-white/10'}`} />
                    </button>
                  ))}
                  <span className="text-sm text-white/40 ml-2 font-bold">{feedbackForm.rating}/5</span>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-2xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-blue-500 transition-colors"
                />
              </div>
        ...
        {activeTab === 'ticket' && (
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-10">
              <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em] mb-3">Get Help</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`p-3 rounded-2xl text-left transition-all border ${ticketForm.issue_type === t.id ? 'bg-blue-600/20 border-blue-500/40 text-white' : 'bg-white/5 border-white/10 text-white/40 hover:border-white/20 hover:text-white/70'}`}
                    >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-2xl pl-8 pr-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-blue-500 transition-colors font-mono"
                  />
                </div>
              </div>
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-2xl text-center transition-all border ${ticketForm.urgency === u.id ? 'bg-white/10 border-white/30 text-white' : 'bg-white/5 border-white/10 text-white/30 hover:text-white/60'}`}
                    >
                      <p className="text-xs font-black">{u.label}</p>
                      <p className="text-[10px] text-white/30 mt-0.5">{u.sub}</p>
                    </button>
                  ))}
                </div>
              </div>
